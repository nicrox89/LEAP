...
class fitness():
    ...
    def __init__(self, d, features,var_type, bounds):
        self.decide = d
        self.stat = []
    ...
    def f(self, chromosome):
        ...
        # ch = np.array(ch.astype('<U30'))

        for j in range(num_genes):
            if self.var_type[j] == 0:
                cutPoint = self.bounds[j][0]+((self.bounds[j][1]-self.bounds[j][0])//2)
                ch[:,j] = [self.binary(x,cutPoint) for x in ch[:,j]]
            else:
                cutPoint = self.bounds[j][0]+((self.bounds[j][1]-self.bounds[j][0])//2)
                ch[:,j] = [self.binaryString(x,cutPoint) for x in ch[:,j]]
